<?php
  ...
  if (sql::select('schedules', 'story_deploy')->where('time=?', [$time_id])->limit(1)->fetch()[0]) {
  ...
  sql::insert('schedules', 'story_deploy')->this([
    $time_id,
    user::post('text'),
    'dict+'.user::post('id')
  ]);
  ...
  foreach ($tag_list as $tag) {
      if (is::in($tag['name'], $tags)) {
          ...
          foreach ($users['data']['openid'] as $user) {
              if (!sql::select('noti_pool', 'story_deploy')->where('time=? and openid=?', [$time_id, $user])->limit(1)->fetch()[0]) {
                  sql::insert('noti_pool', 'story_deploy')->this([
                    $time_id,
                    $user,
                    $tag['name']
                  ]);
  ...
  js::alert('安排上了');
  jump::to(user::url().'/dict/'.user::post('id'));
